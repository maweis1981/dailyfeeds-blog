...
我做的是 Mira（mira.gift），一个 React Native 的 AI 产品。
...
比如 Mira 的图像生成 API、订阅校验 API、用户档案 API，如果都暴露成 MCP，Claude 在 cloud session 里写完代码，就能直接通过 MCP 模拟一个用户跑一遍流程。
...
Max（@maweis1981），独立开发者，Mira（[mira.gift](https://mira.gift)）的 founder。

目前在新加坡，在 NGMOB PTE LTD 旗下用 AI 永动机模式做 AI 产品，同时研究 **AI-Native Engineering** 的工程体系演化。
